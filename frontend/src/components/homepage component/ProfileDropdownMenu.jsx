...
const ProfileDropdownMenu = ({ profilepic, name, role, logout }) => {
  ...
  return (
    <div className="relative w-max mx-auto">
      <button
        ...
        className="px-4 py-2 flex items-center hover:text-black rounded-full  text-sm font-medium border border-slate-300 outline-none hover:bg-slate-200 "
      ...
      >
        <img
          ...
          alt="Profile"
        />
        <span className='hidden sm:inline text-black dark:text-white'>{name} </span>
        <svg
      ...
      {isOpen && (
        ...
        >
          ...
          {
            user?.role === "patient" ? (
              <NavLink to="/patient/profileupdate">
                <li className="dropdown-item py-2.5 px-5 flex items-center hover:bg-slate-200 text-slate-900 text-sm cursor-pointer"
                >Profile Settings</li>
              </NavLink>) : ("")
          }
          {
            user?.role === "doctor" ? (
              <NavLink to="/doctor/updateprofile">
                <li className="dropdown-item py-2.5 px-5 flex items-center hover:bg-slate-200 text-slate-900 text-sm cursor-pointer"
                >Profile Settings</li>
              </NavLink>) : ("")
          }
          ...
          {
            user?.role === "doctor" ? (
              <NavLink to="/doctor/doctordashboard">
                <li className="dropdown-item py-2.5 px-5 flex items-center hover:bg-slate-200 text-slate-900 text-sm cursor-pointer">
                  Dashboard</li>
              </NavLink>) : ("")
          }
